...
class Tenant(Base):
    ...
    kyb_status: Mapped[KybStatus] = mapped_column(String, nullable=False, default=KybStatus.PENDING)
    kyb_company_data: Mapped[dict[str, Any] | None] = mapped_column(JSONB, nullable=True)
    kyb_verified_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
